...
function HeroRosterCard({ entry, isMeta }) {
  ...
  const bestFor = localize(entry.details?.bestFor);
  const bestForItems = Array.isArray(bestFor) ? bestFor : [];
  ...
  return (
    <article className={`hero-roster-card ${isMeta ? 'is-meta' : ''}`}>
      ...
      <div className="hero-roster-body">
        ...
        <p className="hero-roster-summary">{role || localize(entry.summary)}</p>
        ...
        <dl className="hero-roster-meta">
          ...
          {bestForItems.length > 0 && (
            <div>
              <dt>{t('bestFor')}</dt>
              <dd>{bestForItems.slice(0, 2).join(', ')}</dd>
            </div>
          )}
        </dl>
